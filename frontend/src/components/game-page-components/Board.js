...
const Board = ({ addMove, gameId, gameData, isWhite }) => {
  ...
  const rowColToAlgebraic = (row, col) => {
  ...
  const sendMoveToServer = useCallback((fromRow, fromCol, toRow, toCol, promotion = null) => {
    if (!gameId || !socketService.isConnected()) {
      console.warn("Cannot send move: no gameId or WebSocket not connected");
      return;
    }

    // Check if game is in progress
    if (gameData && gameData.status !== 'IN_PROGRESS') {
      console.warn("Cannot send move: game is not in progress yet, status:", gameData?.status);
      return;
    }

    const from = rowColToAlgebraic(fromRow, fromCol);
    const to = rowColToAlgebraic(toRow, toCol);

    // Get username from localStorage for WebSocket auth
    ...
    socketService.publish('/app/move', moveRequest);
  }, [gameId, gameData]);

  // Initialize board from game data
  useEffect(() => {
    if (gameData?.fen) {
      const newBoard = parseFEN(gameData.fen);
      if (newBoard) {
        setBoard(newBoard);
        // Set turn from FEN
  ...
  }, [gameData?.fen, parseFEN]);

  // WebSocket subscription for game updates
  useEffect(() => {
    if (!gameId) return;
    ...
    const handleGameUpdate = (gameState) => {
      ...
      if (gameState.fen) {
        const newBoard = parseFEN(gameState.fen);
        if (newBoard) {
          setBoard(newBoard);
          setIsWhiteTurn(gameState.currentTurn === 'WHITE');
        }
      }
    };

    // Connect and subscribe using centralized socket service
    socketService.connect()
      .then(() => {
        console.log("WebSocket connected, subscribing to game:", gameId);
  ...
  const getPieceIcon = (piece) => {
    let imgName = "";
    switch (piece) {
      case 'r': imgName = 'bR'; break;
      case 'n': imgName = 'bN'; break;
      case 'b': imgName = 'bB'; break;
      case 'q': imgName = 'bQ'; break;
      case 'k': imgName = 'bK'; break;
      case 'p': imgName = 'bP'; break;
      case 'R': imgName = 'wR'; break;
      case 'N': imgName = 'wN'; break;
      case 'B': imgName = 'wB'; break;
      case 'Q': imgName = 'wQ'; break;
      case 'K': imgName = 'wK'; break;
      case 'P': imgName = 'wP'; break;
      default: return null;
    }
    // Using simple concatenation if images are named wP.png etc.
    return <img src={`/pieces/${imgName}.png`} alt={piece} className="piece-img" draggable={false} />;
  };
  ...
  const handleDrop = (e, row, col) => {
    ...
    if (validMoves.some(([r, c]) => r === row && c === col)) {
      ...
      // Send move to server via WebSocket for multiplayer sync
      sendMoveToServer(fromRow, fromCol, row, col, null);
    }
    setIsSquareSelected(false);
    setSelectedSquare([]);
    setValidMoves([]);
  ...
  return (
    <div className="chessboard-container">
      <div
        ...
        style={{ width: '100%', height: '100%', maxWidth: '100vh', maxHeight: '100vh', aspectRatio: '1/1' }}
      >
        {/* Render the board */}
